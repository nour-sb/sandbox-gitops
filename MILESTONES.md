...
### L4 — Terminal Commands Have No CLI Auto-Approve Flag
`grafana-assistant tunnel connect --terminal` has no `--auto-approve` flag and no command allowlist. However, Grafana's per-tool approval settings (same UI as custom MCP tools) apply to tunnel tools — setting `terminal_execute` to auto-approve in Grafana eliminates the human click requirement. Without auto-approve configured in Grafana, every terminal call blocks on a human click and triggers PLR (see L1).
...
The Slack thread is the AI's context window — any user who posts @Grafana in the thread continues the same conversation with full prior context. However, the conversation record is attributed entirely to whoever first triggered @Grafana. grafana-test posted @Grafana in the thread and got a full AI response, but had zero conversation records in their assistant history. Programmernour (first caller) owns the conversation log. Billing likely charges the first caller for the entire thread, not subsequent participants.

**M8-F3 — Disposable email domains blocked by Grafana Cloud**
Grafana Cloud blocks known disposable email domains (guerrillamail, mailinator, yopmail) at the org invite stage with a misleading "already a member" error. A custom domain with real MX records was required.
