...
### L4 — No Auto-Approve for Terminal Commands
`tunnel connect --terminal` always requires UI approval per command. No `--auto-approve`, no command allowlist, no pre-authorization. Every terminal call blocks on a human click.
